Warning: truncated output (original token count: 55031)
Total output lines: 4324
...
function getCountComparison(key, baseValue) {
  ...
  return { ...baseline, actual: Number(baseValue), difference, threshold, significant: Math.abs(difference) >= th…5031 tokens truncated…answered_by: pilot.profile.id, answered_at: new Date().toISOString()
      });
    } catch (error) {
