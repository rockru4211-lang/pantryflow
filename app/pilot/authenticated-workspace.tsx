...
function WorkspaceContent({session,profile,stores,selectedStoreId,versionPanel,onStoreChange,onChanged,onSignOut,onChangePassword,demo=false,registerLeave}:Props){
  ...
  async function openExpiry(page:ExpiryWastePage,from:ShellView=view,id?:string,month?:string){
    ...
    setExpiryId(id);setTargetMonth(month);setExpiryStartPage(page);
  ...
  const workspace=()=>{
    ...
    if(view==='business'&&businessEntry==='home')return <PartnersStoresWorkspace anchorStore={selectedStore} stores={stores.filter(canManageStores)} onBack={()=>backTo('settings')} onOpenPartners={async id=>{if(id!==selectedStoreId)await changeStore(id);setOrigins(o=>({...o,members:'business'}));setView('members');}} onOpenStore={async id=>{if(id!==selectedStoreId)await changeStore(id);setBusinessEntry('store-home');setView('business');}}/>;
    ...
    if(view==='members'||view==='permissions')return <MembersWorkspace returnLabel={origins[view]==='business'?'返回夥伴與門市':`返回${viewTitles[origins[view]||"settings"]||"上一頁"}`} key={`${selectedStoreId}:${view}`} store={selectedStore} userId={session.user.id} section={view} onBack={()=>backTo('settings')} onChanged={onChanged}/>;
