...
export default function ChangePasswordForm({onChangePassword}:{onChangePassword:(current:string,next:string)=>Promise<string|null>}){
 ...
 return <details className="shell-card password-change"><summary>變更密碼</summary><form className="admin-login-form" onSubmit={submit}>
  <label className="field" htmlFor="change-current-password">目前密碼<PasswordInput aria-label="目前密碼" id="change-current-password" name="current_password" autoComplete="section-change current-password" required/></label>
  <label className="field" htmlFor="change-next-password">新密碼<PasswordInput aria-label="新密碼" strength id="change-next-password" name="next_password" autoComplete="section-change new-password" minLength={8} required/></label>
  <label className="field" htmlFor="change-confirm-password">再次輸入新密碼<PasswordInput aria-label="再次輸入新密碼" id="change-confirm-password" name="confirm_password" autoComplete="section-change new-password" minLength={8} required/></label>
