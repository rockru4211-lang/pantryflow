...
export default function InventoryImportFlow({userId,storeId,organizationId,disabled,onImported}:{userId:string;storeId:string;organizationId:string;disabled:boolean;onImported:()=>Promise<void>}) {
 ...
 async function loadPersisted(file:ImportSource,quiet=false){
  ...
  const built=(result.data||[]).filter(r=>r.product_id&&['ADDED','EXISTING'].includes(String(r.status))).map(r=>{const product=products.get(String(r.product_id));const review=bySource.get(String(r.source_id));return {productId:String(r.product_id),sourceId:String(r.source_id),name:product?.name||review?.name||'未命名',unit:product?.count_unit||review?.unit||'未設定',specification:product?.specification??review?.specification??null,zone:review?.zoneName||'未分類',quantity:review?.quantityText||'未提供',status:String(r.status) as ReviewStatus,reason:String(r.reason||review?.reason||''),updated_at:product?.updated_at};});
  setBuiltItems(built);
  if(!quiet&&built.length)setNotice(`已恢復上次進度：${built.length} 個品項已建立。`);
 }

 useEffect(()=>{if(booted.current)return;booted.current=true;void(async()=>{try{const latest=await fetchFiles();if(latest[0])await loadPersisted(latest[0]);}catch{}})();},[storeId]);
 ...
 const needsFix=(item:BuiltItem)=>item.zone==='未分類'||item.quantity==='未提供'||item.unit==='未設定'||!item.specification||Boolean(item.reason);
 ...
 const openItem=(item:BuiltItem)=>{setSelected(item);setScreen('item');};
 ...
 return <div ref={rootRef} className="inventory-import-flow">
  {screen==='main'&&<>
   <div className="shell-card" style={{padding:'12px 10px',marginBottom:12}}><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,textAlign:'center',fontSize:12}}>{['上傳資料','系統整理','開始盤點'].map((label,index)=>{const active=index===0?!builtItems.length:index===1?busy:index===2?builtItems.length>0:false;return <div key={label}><div style={{width:28,height:28,borderRadius:99,margin:'0 auto 6px',display:'grid',placeItems:'center',background:active||index===0&&source?'#0f513f':'#d9dfdc',color:'white',fontWeight:700}}>{index+1}</div><span>{label}</span></div>;})}</div></div>
   {!builtItems.length&&<section className="shell-card upload-shell"><h2>上傳盤點資料</h2><p>拍照或上傳檔案，系統會自動辨識品名與手寫期初數字。</p><label className="import-button">{busy?'系統整理中…':'選擇檔案或照片'}<input type="file" accept=".xlsx,.xls,.csv,.pdf,.jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" disabled={busy} onChange={e=>{const f=e.target.files?.[0];e.target.value='';if(f)void readFile(f);}}/></label><small className="shell-note">支援 Excel、CSV、PDF、JPG、PNG、WEBP</small><div className="shell-card" style={{marginTop:14,padding:12,background:'#f6f8f7'}}><strong>小提示</strong><p className="shell-note">可上傳盤點表、手寫單等，系統會自動辨識與整理。</p></div></section>}
   {builtItems.length>0&&<section className="shell-card" style={{padding:18,textAlign:'center'}}><div style={{width:58,height:58,borderRadius:99,margin:'0 auto 12px',display:'grid',placeItems:'center',background:'#dff3e8',fontSize:30,color:'#0f513f'}}>✓</div><h2>資料已整理完成</h2><p className="shell-note">系統已自動辨識、比對並建立品項，您可以直接開始盤點。</p><div style={{display:'grid',gap:8,margin:'16px 0'}}><div style={{padding:11,borderRadius:12,background:'#eef8f3'}}><strong>{builtItems.length}</strong> 個品項已建立</div><div style={{padding:11,borderRadius:12,background:'#eef8f3'}}><strong>{ready.length}</strong> 個可直接使用</div><div style={{padding:11,borderRadius:12,background:'#fff4e7'}}><strong>{exceptions.length}</strong> 個需後續確認</div></div><button className="shell-primary full" onClick={()=>void enterCount()}>開始盤點 →</button>{exceptions.length>0&&<button className="shell-secondary full" style={{marginTop:8}} onClick={()=>setScreen('exceptions')}>查看需確認的 {exceptions.length} 筆</button>}<p className="shell-note" style={{marginTop:10}}>不需要現在全部確認，現場可先開始盤點。</p></section>}
  ...
  {screen==='item'&&selected&&<><button className="shell-back" onClick={()=>setScreen('exceptions')}>‹ 返回需確認項目</button><h1>{selected.name}</h1><section className="shell-card" style={{padding:14}}><div style={{display:'grid',gridTemplateColumns:'72px 1fr',gap:'10px 12px'}}><span>品名</span><b>{selected.name}</b><span>單位</span><b>{selected.unit}</b><span>規格</span><b>{selected.specification||'待補'}</b><span>儲物區</span><b>{selected.zone}</b><span>期初</span><b>{selected.quantity}</b></div></section><div style={{marginTop:12}}><ProductBasicEditor storeId={storeId} userId={userId} product={{id:selected.productId,name:selected.name,count_unit:selected.unit,specification:selected.specification,updated_at:selected.updated_at}} onSaved={(product:BasicProduct)=>{setBuiltItems(current=>current.map(row=>row.productId===product.id?{...row,name:product.name,unit:product.count_unit,specification:product.specification,updated_at:product.updated_at}:row));setSelected(current=>current?{...current,name:product.name,unit:product.count_unit,specification:product.specification,updated_at:product.updated_at}:current);}}/></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginTop:12}}><button className="shell-secondary" onClick={()=>void excludeItem(selected)}>本次略過</button>{selected.status==='ADDED'?<button className="text-button" style={{color:'#b42318'}} onClick={()=>void removeItem(selected)}>移除品項</button>:<button className="text-button" disabled>保留既有品項</button>}</div><p className="shell-note" style={{marginTop:10}}>「本次略過」只排除本次盤點；「移除品項」僅適用本次新建且尚未盤點的品項。</p>{notice&&<p className="pilot-message" role="status">{notice}</p>}</>}
 </div>;
}
