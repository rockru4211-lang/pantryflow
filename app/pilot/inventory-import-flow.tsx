...
export default function InventoryImportFlow({userId,storeId,organizationId,disabled,onImported,onHistory}:{userId:string;storeId:string;organizationId:string;disabled:boolean;onImported:()=>Promise<void>;onHistory?:()=>void}) {
 ...
 return <div ref={rootRef} className="inventory-import-flow">
  {screen==='main'&&<>
   <div className="shell-card" style={{padding:'12px 10px',marginBottom:12}}><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,textAlign:'center',fontSize:12}}>{['上傳資料','系統整理','開始盤點'].map((label,index)=>{const stage=builtItems.length?3:busy?2:1;const done=index+1<=stage;return <div key={label} style={{opacity:done?1:.4}}><div style={{width:28,height:28,borderRadius:99,margin:'0 auto 6px',display:'grid',placeItems:'center',background:done?'#0f513f':'#d9dfdc',color:'white',fontWeight:700}}>{index+1}</div><span>{label}</span></div>;})}</div></div>
