...
import {catalogStates,importSummaryLabel,summarizeImportRows,type CatalogState,type CatalogStatus} from '@/lib/inventory-import-status';
...
type BuiltItem={file:ImportSource;productId:string;sourceId:string;name:string;unit:string;specification:string|null;zone:string;quantity:string;status:ReviewStatus;reason?:string;updated_at?:string;isActive?:boolean;catalogState:CatalogState};
...
export default function InventoryImportFlow({storeId,storeName,organizationId,disabled,onImported,onHistory,onStartCount,onOrganize}:{storeId:string;storeName?:string;organizationId:string;disabled:boolean;onImported:()=>Promise<void>;onHistory?:()=>void;onStartCount?:()=>void;onOrganize?:()=>void}) {
 ...
 const countableItems=builtItems.filter(item=>item.catalogState==='ACTIVE');
 ...
 function enterCount(){if(!busy&&!disabled&&countableItems.length)onStartCount?.();}
 ...
 const builtStates=new Map(builtItems.map(item=>[item.productId,item.catalogState]));
 const fileSummary=(hash:string)=>summarizeImportRows(builtItems.filter(item=>item.file.file_sha256===hash).map(item=>({product_id:item.productId,status:item.status})),builtStates);
 ...
 return <div className="inventory-import-flow">
  {screen==='main'&&<>
   <section className="shell-card" style={{padding:18}}>
    ...
    {builtItems.length>0?<>
     <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,margin:'14px 0'}}><div style={{padding:14,borderRadius:14,background:'#eef8f3',textAlign:'center'}}><strong style={{fontSize:28}}>{new Set(countableItems.map(item=>item.productId)).size}</strong><small style={{display:'block'}}>可盤點品項</small></div><div style={{padding:14,borderRadius:14,background:'#fff4e7',textAlign:'center'}}><strong style={{fontSize:28}}>{exceptions.length}</strong><small style={{display:'block'}}>需確認項目</small></div></div>
     {onOrganize&&<button className="shell-primary full" disabled={locked} onClick={onOrganize}>整理品項與儲物區 →</button>}{onStartCount&&<button className="shell-secondary full" disabled={locked||!countableItems.length} onClick={enterCount}>開始盤點</button>}
    ...
    {uploadButton}
    {importedFiles.length>0&&<><p className="shell-note" style={{marginTop:14}}>已加入 {importedFiles.length} 份檔案・左滑可整批移除資料與品項</p><div className="swipe-list">{importedFiles.map(file=>{const summary=fileSummary(file.file_sha256);return <div className="swipe-row" key={file.file_sha256}><div className="swipe-row-main"><span><strong>{file.original_filename}</strong><small>已建檔 {summary.builtRows} 列</small><small>{importSummaryLabel(summary)}</small></span></div><button type="button" className="swipe-row-remove" disabled={locked} aria-label={`整批移除資料與品項：${file.original_filename}`} onClick={()=>void undoBatch(file)}><Trash2 size={20} aria-hidden="true"/><span>移除</span></button></div>;})}</div></>}
