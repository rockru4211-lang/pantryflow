...
type Props={userId:string;store:AppStore;stores:AppStore[];onStoreChange:(id:string)=>void;registerLeave?:(handler:(()=>Promise<boolean>)|null)=>void};
const dateLabel=(value:string|null|undefined)=>value?new Date(value).toLocaleString('zh-TW',{timeZone:'Asia/Taipei',hour12:false}):'—';
export default function InventoryMonthlyWorkspace({userId,store,stores,onStoreChange,registerLeave}:Props) {
 ...
 const custody=tab==='supplier'||tab==='reserved';
 ...
 const rows=visible?.rows||[];const categoryRows=category?rows.filter(r=>r.category===category):rows;const filtered=filterInventory(rows,{search,zone,category,pending:pending||tab==='review'}),pages=Math.max(1,Math.ceil(filtered.length/20)),currentPage=Math.min(page,pages),pageRows=filtered.slice((currentPage-1)*20,currentPage*20);
 ...
 const summary=visible?.summary,displaySummary=category&&visible?inventoryCategorySummary(categoryRows,visible.has_previous):summary,partial=!!displaySummary&&(displaySummary.missing_prices>0||displaySummary.previous_missing_prices>0);
 ...
 return <section className="inventory-monthly" aria-label="庫存管理">
  ...
  <div className="im-selectors"><label>門市<select value={store.id} disabled={busy} onChange={e=>onStoreChange(e.target.value)}>{stores.filter(s=>['LOGISTICS','OWNER'].includes(s.role)&&s.is_active!==false).map(s=><option value={s.id} key={s.id}>{s.name}</option>)}</select></label>{!custody&&<><label>盤點月份<input type="month" min="2000-01" max="2100-12" value={month} disabled={busy} onChange={e=>void switchMonth(e.target.value)}/></label><span className={`im-badge ${visible?.closed?'done':''}`}>{visible?.closed?'本月已確認':'本月待確認'}</span><small>{editor?'核對中，保留您的輸入':lastRead?`每 5 秒同步・${lastRead}`:'讀取資料中'}</small></>}</div>
  {!custody&&<div className="im-cards"><article><span>{category?`${category}庫存金額`:'本月庫存金額'}</span><strong>{inventoryMoney(displaySummary?.subtotal)}</strong><small>{displaySummary?.missing_prices?`已計價小計・${displaySummary.missing_prices} 項缺單價未計入`:`${displaySummary?.items??0} 個品項／單位・依所選盤點範圍`}</small></article><article><span>較上月金額增減</span><strong>{inventoryMoney(displaySummary?.amount_difference,true)}</strong><small>{!visible?.has_previous?'無上月資料，暫不比較':partial?'部分品項未計價，僅比較已計價金額':`對照 ${visible.previous_month.slice(0,7)}${visible.previous_closed?' 已確認資料':' 未確認盤點'}`}</small></article><article className="im-review-card"><span>待核對項目</span><strong>{displaySummary?.pending??'—'} <small>項</small></strong><button onClick={()=>{setTab('review');setPage(1);}}>查看需要核對的項目 →</button></article></div>}
  ...
  {loading&&!visible?<p className="im-empty" role="status">正在讀取盤點明細…</p>:!visible?.source_id?<div className="im-empty"><Warehouse/><h2>這個月份尚無已完成盤點</h2><p>請選擇其他月份，或待門市完成盤點後再查看。</p></div>:<>
   ...
   <CategoryFilter value={category} disabled={busy||!!editor} onChange={value=>{setCategory(value);setPage(1);setExpanded(null);}}/><div role="tabpanel" id={`im-panel-${tab}`} aria-label={tab==='total'?'總表':tab==='review'?'差異核對':'庫存金額'}>
   {tab==='amount'?<><p className="im-note">各分類依品項合計，跨儲物區不重複計價。缺少單價的品項未計入金額。</p><div className="im-table-wrap"><table><thead><tr><th>分類</th><th>品項／單位數</th><th>庫存金額</th><th>計價狀態</th></tr></thead><tbody>{inventoryCategories(categoryRows).map(g=><tr key={g.name}><td>{g.name}</td><td>{g.items}</td><td>{inventoryMoney(g.amount)}</td><td>{g.missing?`${g.missing} 項待補單價`:'已完整計價'}</td></tr>)}</tbody></table></div></>:<>
    ...
    <div className="im-table-wrap"><table><thead><tr><th>品項</th><th>分類</th><th>儲物區</th><th>單位</th><th>上月數量</th><th>本月數量</th><th>增減</th><th>單價</th><th>本月金額</th><th>核對狀態</th></tr></thead><tbody>{pageRows.map(row=><InventoryTableRows key={row.row_key} row={row} categoryControl={<CategorySelect storeId={store.id} userId={userId} id={row.product_id} name={row.name} value={row.category} revision={row.category_revision||0} disabled={busy||loading||!!editor} onBusy={value=>{working.current=value;request.current++;setBusy(value);}} onSaved={(category,revision)=>setData(old=>old?{...old,rows:old.rows.map(r=>r.product_id===row.product_id?{...r,category,category_revision:revision}:r)}:old)}/>} expanded={expanded===row.row_key} onExpand={()=>setExpanded(expanded===row.row_key?null:row.row_key)} onEdit={()=>edit(row)} disabled={busy||loading} closed={visible.closed}/>)}{!pageRows.length&&<tr><td colSpan={10} className="im-empty">{tab==='review'?'目前沒有符合條件的待核對項目':'沒有符合條件的品項'}</td></tr>}</tbody></table></div>
...
export function InventoryTableRows({row,categoryControl,expanded,onExpand,onEdit,disabled,closed}:{categoryControl?:ReactNode;row:InventoryRow;expanded:boolean;onExpand:()=>void;onEdit:()=>void;disabled:boolean;closed:boolean}) {return <><tr><td><strong>{row.name}</strong><small>{row.supplier||'未提供供應商'}</small></td><td>{categoryControl||row.category}</td><td><button className="im-zone-toggle" aria-expanded={expanded} onClick={onExpand}>{row.zones.length>1?`${new Set(row.zones.map(z=>z.zone_id)).size} 個儲物區`:row.zones[0]?.zone||'未提供'}<ChevronDown size={14}/></button></td><td>{row.unit}</td><td>{inventoryNumber(row.previous_quantity)}</td><td><strong>{row.current_quantity===null?'未盤':inventoryNumber(row.current_quantity)}</strong>{row.corrected&&<small>含合計更正</small>}</td><td className={row.difference?'im-difference':''}>{comparisonLabel(row)}</td><td>{inventoryNumber(row.unit_price)}</td><td>{row.amount===null?'未計入':inventoryMoney(row.amount)}</td><td><button disabled={disabled} onClick={onEdit} className={`im-status ${row.needs_review?'pending':'done'}`}>{closed?'查看紀錄':reviewLabel(row)}</button></td></tr>{expanded&&<tr><td colSpan={10}><ZoneDetails row={row}/></td></tr>}</>;}
