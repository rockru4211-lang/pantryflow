...
export function useOperation(storeId:string,userId:string){
 ...
 const run=async<T=Record<string,unknown>>(action:string,data:Record<string,unknown>):Promise<T|undefined>=>{
  ...
  try{const result=await (action==='supplier.resolve-name'||action.startsWith('receipt.')||action.startsWith('count.')||action==='product.edit-basic'||action==='product.category'?operationDeadline(signal=>writeOperation<T>(storeId,action,data,id,signal)):writeOperation<T>(storeId,action,data,id));pending.current.delete(key);try{workspaceStorage(userId).removeItem(key);}catch{}return result;}catch(e){setError(appError(e));return undefined;}finally{lock.current=false;setBusy(false);}
 };
