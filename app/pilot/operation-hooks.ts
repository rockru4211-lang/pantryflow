...
export function useOperation(storeId:string,userId:string){
 ...
 const run=async<T=Record<string,unknown>>(action:string,data:Record<string,unknown>):Promise<T|undefined>=>{
  ...
  try{const old=JSON.parse(localStorage.getItem(key)||'null');if(old?.signature===signature&&typeof old.id==='string')id=old.id;localStorage.setItem(key,JSON.stringify({id,signature}));}catch{/* Retry token still works in memory for the in-flight request. */}
  pending.current.set(key,{id,signature});
  try{const result=await writeOperation<T>(storeId,action,data,id);pending.current.delete(key);try{localStorage.removeItem(key);}catch{}return result;}catch(e){setError(appError(e));return undefined;}finally{lock.current=false;setBusy(false);}
 };
...
export function useOperationDraft<T>(userId:string,storeId:string,name:string,empty:T){
 ...
 useEffect(()=>{try{const saved=localStorage.getItem(key);setValue(saved?JSON.parse(saved):initial.current);}catch{setValue(initial.current);}setLoadedKey(key);},[key]);
 useEffect(()=>{if(loadedKey===key)try{localStorage.setItem(key,JSON.stringify(value));}catch{}},[key,loadedKey,value]);
 const clear=()=>{setValue(initial.current);try{localStorage.removeItem(key);}catch{}};
