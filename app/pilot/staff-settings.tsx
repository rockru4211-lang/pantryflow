...
export default function StaffSettings({stores,allowedRoles=['STAFF'],onWorkspaceChanged,businessType='SINGLE_RESTAURANT'}:{stores:{id:string;name:string;store_code:string;staff_login_mode?:string}[];canManageStores:boolean;allowedRoles?:AppRole[];createOnly?:boolean;onWorkspaceChanged:()=>Promise<void>;businessType?:string}){
 ...
 async function create(event:FormEvent<HTMLFormElement>){
  ...
  try{
   const {data,error}=await supabase.functions.invoke<Result>('manage-staff',{body:{action:emailInvite&&role!=='STAFF'?'invite_management':'create',storeId:store.id,displayName:name,role,email:String(values.get('email')||'').trim(),loginIdentifier:store.staff_login_mode==='EMPLOYEE_NUMBER'?'':name}});
   ...
   setReceipt(data.login);setMessage(data.login?'成員已建立，請將邀請連結或 QR Code 交給本人設定 PIN。':data.pending?'邀請已保留；本人可使用原帳號登入並確認加入。':`邀請信已寄至 ${data.email}，請本人確認加入。`);form.reset();await onWorkspaceChanged();
  }catch{setMessage('目前無法完成，輸入內容已保留，請稍後再試。');}finally{lock.current=false;setBusy(false);}
 }
 return <section className="shell-section"><h2>新增成員</h2><p>{store?.name}</p><article className="shell-card settings-card"><form className="compact-form" onSubmit={create}><label>姓名／暱稱<input name="display_name" required maxLength={64}/></label><label>工作身分<select value={role} onChange={e=>{setRole(e.target.value as AppRole);setEmailInvite(false);}}>{allowedRoles.map(r=><option key={r} value={r}>{roleLabel(r,businessType)}</option>)}</select></label>{role!=='STAFF'&&<details className="setup-panel"><summary>使用 Email 邀請（選用）</summary><label className="checkbox-row"><input type="checkbox" checked={emailInvite} onChange={e=>setEmailInvite(e.target.checked)}/>使用個人 Email 帳號加入</label>{emailInvite&&<label>Email<input name="email" type="email" autoComplete="off" autoCapitalize="none" required/></label>}</details>}<p className="shell-note">先套用身分預設權限，需要時可從成員資料調整。</p><button className="shell-primary full" disabled={busy}>{busy?'建立中…':'建立成員'}</button></form></article>{message&&<p className="count-notice" role="status">{message}</p>}{receipt&&<article className="shell-card settings-card login-receipt" aria-label="新帳號登入資料"><h3>{receipt.displayName}</h3><p>{receipt.storeCode}・{roleLabel(receipt.role as AppRole,businessType)}</p><p>之後登入使用：{receipt.loginIdentifier}</p><StaffInvitationCard token={receipt.activationCode} days={receipt.expiresInDays}/></article>}</section>;
}
