...
export default function StaffSettings({stores,allowedRoles=['STAFF'],onWorkspaceChanged,businessType='SINGLE_RESTAURANT'}:{stores:{id:string;name:string;store_code:string;staff_login_mode?:string}[];canManageStores:boolean;allowedRoles?:AppRole[];createOnly?:boolean;onWorkspaceChanged:()=>Promise<void>;businessType?:string}){
 const store=stores[0];const[role,setRole]=useState<AppRole>('STAFF');const[busy,setBusy]=useState(false);const[message,setMessage]=useState('');const[receipt,setReceipt]=useState<Login>();const lock=useRef(false);
 const useManagementLogin=role==='LOGISTICS'||role==='OWNER';
 ...
 return <section className="shell-section"><h2>新增夥伴</h2><p>{store?.name}</p><article className="shell-card settings-card"><form className="compact-form member-create-form" onSubmit={create}><label>姓名／暱稱<input name="display_name" required maxLength={64}/></label><label>主要身分<select value={role} onChange={e=>setRole(e.target.value as AppRole)}>{allowedRoles.map(r=><option key={r} value={r}>{roleLabel(r,businessType)}</option>)}</select></label>{(role==='STAFF'||role==='SUPERVISOR')&&<p className="shell-note">員工與主管使用相同的門市現場登入：門市代碼、個人識別與 6 位 PIN。主管若同時負責行政／營運，不需要再建立第二個帳號；由 Owner 另外開啟行政管理權限即可。</p>}{useManagementLogin&&<><p className="shell-note">行政／後勤與 Owner 使用管理登入，不建立門市 PIN。</p><label>Email<input name="email" type="email" autoComplete="off" autoCapitalize="none" required/></label></>}<p className="shell-note">登入方式與權限分開管理；工作身分可再從成員資料調整。</p><button className="shell-primary full" disabled={busy}>{busy?'建立中…':useManagementLogin?'寄送管理邀請':'建立夥伴帳號'}</button></form></article>{message&&<p className="count-notice" role="status">{message}</p>}{receipt&&<article className="shell-card settings-card login-receipt" aria-label="新帳號登入資料"><h3>{receipt.displayName}</h3><p>{receipt.storeCode}・{roleLabel(receipt.role as AppRole,businessType)}</p><p>現場登入使用：{receipt.loginIdentifier}</p><StaffInvitationCard token={receipt.activationCode} days={receipt.expiresInDays}/></article>}</section>;
}
