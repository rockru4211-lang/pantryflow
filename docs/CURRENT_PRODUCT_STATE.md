...
- `pilot-v1/` 共用米白底、白卡片、森林綠頂欄／主按鈕與四欄行動導覽；全域沒有掃描、條碼或中央浮動按鈕。
- Owner／ADMIN／SUPERVISOR／LOGISTICS 使用 Email 與密碼；STAFF 只顯示主管建立的「姓名／暱稱＋6 位 PIN」或「員工編號＋6 位 PIN」入口。
- Owner 註冊明確分為 Email 驗證、建立商家、第一間門市與員工登入方式設定；所有完成動作仍呼叫正式 Supabase Auth 與 onboarding RPC。
- 同一首頁版型依角色調整焦點文字、快速入口與輔色，不切換第二套 UI。沒有正式作業資料時以「封閉 Pilot 建置狀態」呈現，所有摘要維持不確定符號，不產生假數字。
- 此切片僅完成可視化登入與首頁；未取得正式角色帳號及跨角色截圖前，不得標示為可測或現場可用。
- 視覺驗收中的首頁固定採三欄深綠頂欄（門市切換／PantryFlow／通知），內容順序為「今日重點／每日作業／需要處理」，底部固定「首頁／作業紀錄／通知／我的」。管理首頁只在商家正式啟用 ERP 驗收時呈現 ERP 待處理列；沒有正式資料時不顯示數字。
- 完整性修復以 `docs/PILOT_ACCEPTANCE_MATRIX.md` 為 Gate；目前整體仍為「封閉 Pilot／不可測」。第一依賴層已在正式 `profile` route 接上門市成員清單、建立員工／主管、重設 PIN 與停用操作，全部呼叫正式 Supabase RLS 查詢及 `manage-staff` Edge Function；在真實帳號與跨裝置證據完成前仍只列部分完成。
