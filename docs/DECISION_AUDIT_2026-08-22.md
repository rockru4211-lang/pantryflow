...
| ADMIN 可新增、改名、排序區域；商品可批量加入多區域。 | 使用者受控 Pilot定案，2026-08-22；`DECISIONS_2026-08-21` §3 | **部分符合** | 正式 CRUD 與批量加入 UI 存在；沒有完整真實驗收。簡單數字排序符合本輪不要求拖移。 | **P1** | 不得把拖移排序當本輪必要條件，也不得用 local 區域資料驗收。 |
...
| 寄庫本輪只保留正式「下一輪」設計，不建立假功能。 | `DECISION_LOG` 2026-08-22；`CONTROLLED_PILOT_2026-08-23` | **符合** | 未發現可點擊的假成功寄庫流程。 | **P2** | 封存文件中的廣泛庫存生命週期不能被誤認本輪已上線。 |
