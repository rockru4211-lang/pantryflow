...
| PF-PILOT-FUNCTIONAL-REPAIR-20260827 | 2026-08-27 | [`PF-PILOT-FUNCTIONAL-REPAIR-20260827.md`](decisions/PF-PILOT-FUNCTIONAL-REPAIR-20260827.md)／主管 production 實測 | ADMIN、SUPERVISOR、STAFF、進貨核對者 | session 失效 → 重登；區域完整設定；CSV／XLSX 預覽 → 正式匯入 → 原位置回填；多圖 → OCR → 查看／重試 | 開發中 | `fix/pilot-test-functional-20260827`；Node 66／66；migration 待綁定 merged main SHA 後套用 |
