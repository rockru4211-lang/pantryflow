...
最後更新：2026-08-23
狀態：已授權依序實作；完成狀態仍以正式驗收為準
產品狀態：**內部整合中，不可現場使用**

本計畫處理 `DECISION_AUDIT_2026-08-22.md` 的七項基礎 P0，並依 `CLOSED_PILOT_SCOPE_2026-08-23.md` 擴充為下週封閉 Pilot 的七個依序 Release Gate。每日進貨工作台、固定模板 Excel 商品匯入、限定範圍拖移、寄庫與基本彙整／匯出均為 Pilot 必含，不再列為下一輪；仍禁止外觀擴充、mock、seed、localStorage 與假成功訊息。

## 2026-08-23 重新拆分後的 P0

| Gate | 範圍 | 主要相依 | 完成定義 |
|---|---|---|---|
| P0-A | 登入、Store membership、PIN、角色與門市 RLS | 既有 actor FK | 真實 ADMIN／主管／STAFF、正向與跨店／跨商家負向測試全過 |
| P0-B | PantryFlow 固定 Excel 模板與正式 catalog 匯入 | P0-A | 下載、預覽、錯誤列、重複策略、transaction 寫入與回查全過 |
| P0-C | 區域排序與商品跨區加入／移動／移除／排序 | P0-A、P0-B | 桌機與手機基本操作寫正式 Supabase，重載一致且歷史未刪 |
| P0-D | baseline gate、真實盲盤、任務／差異／操作者 | P0-A～C | baseline 優先順序、0／空白／小數、多區加總、跨裝置與 RLS 全過 |
| P0-E | 多圖私有上傳、背景 OCR、每日進貨工作台與 ERP 手動狀態 | P0-A、正式 catalog | 原圖／run／修正／正式收貨可追溯，稅額分離且可跨裝置回查 |
| P0-F | 供應商寄庫與 append-only 領用 ledger | P0-A、P0-B | 寄庫餘額、領用人／日、低庫存正確，且與店內庫存完全分離 |
| P0-G | 盤點表列印、進貨與差異匯出、基本彙整 | P0-D～F | 全部從正式 Supabase 重建、不改原始資料，內容與畫面／SQL 對帳一致 |

優先順序固定為 `P0-A → P0-B → P0-C → P0-D → P0-E → P0-F → P0-G`。詳細每日日期、欄位與排除項見 `docs/CLOSED_PILOT_SCOPE_2026-08-23.md`。
...
- 已獲授權依 P0 相依順序實作，但每個 Gate 仍須先完成 migration／RLS 設計與可回復方案，不得跳過資料安全。
- Excel 商品匯入、限定範圍區域／商品拖移、每日進貨工作台、寄庫與基本匯出屬封閉 Pilot 必含；只能在前置 Gate 通過後接正式 Supabase 實作，不得以展示資料並行假造。
