...
- [ ] 手機在上傳與排程成功後即可離開，不等待 Gemini；job 最多兩件並行，失敗有限重試且可重跑。
- [ ] OCR job 過期租約可接手；一般 authenticated／anon 無法直接 claim、完成、失敗或刪除 job。
- [ ] 每日收貨工作台可依門市、實際收貨日、供應商、狀態篩選，預設排除已完成且「查看全部」可追溯歷史。
- [ ] 後勤可查看原圖、AI 原值、人工修正，並以單一正式交易完成收貨。
- [ ] 每次重跑建立新 OCR run；供應商／貨單分組不會合併不同 batch、供應商、稅額或原圖。
