...
| 四種角色首頁 | STAFF 綠、ADMIN／SUPERVISOR 橘、LOGISTICS 藍、OWNER 紫；區塊順序、卡片與數字依圖；五欄導覽為首頁／作業紀錄／待辦／通知／我的；桌機仍維持 390px 手機操作畫布，不拉成 Dashboard | 登入後依真實角色直接分流；「待辦」內容依角色；後勤與 Owner 不得借用店長首頁；資料先用安全範例，待 Supabase 階段串接 | PF-HOME-ROLES-FINAL-20260827 | 視覺對齊開發中；`fix/preview-pages-visual-parity-20260827`，Node 68／68 |
...
| 原盤點表自動識別修復 | 維持店長／主管橘色身份與原盤點管理頁；選檔後顯示建立中或完成結果，只有識別衝突才顯示待處理內容 | 不要求商品碼；「編號」只作原表順序；依品名產生穩定識別碼；多工作表一起辨識；`期末數量`優先作回填欄；安全辨識後直接建立區域、商品與盤點細項；同區同品重複列合併，識別衝突時整批不寫入 | PF-PILOT-COUNT-RECEIVING-FINAL-20260827／PF-PILOT-FUNCTIONAL-REPAIR-20260827 | `fix/count-import-recognition-20260831` 開發中；Node 78／78；未合併、未部署 |
