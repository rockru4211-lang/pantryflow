...
- Treat manual deactivation, store removal and rebuilding as separate lifecycle cases. Verify that batch/single removal leaves other stores and the global activation state unchanged, that rebuilding does not override manual deactivation, and that catalog/import totals distinguish countable, disabled, removed and failed records.
