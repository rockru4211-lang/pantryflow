...
export function authErrorMessage(error: { code?: string; message?: string }, flow?: AuthFlow | null) {
  ...
  if (code === "over_email_send_rate_limit" || /email rate limit/i.test(message)) return "寄信服務暫時達到寄送上限，請稍後重新寄送。";
  ...
  if (code === "otp_expired" || code === "flow_state_expired" || /expired|invalid.*token|Token.*invalid/i.test(message)) return flow === "recovery" ? "重設連結已過期或已使用，請重新寄送重設信。" : "驗證連結或驗證碼已過期或已使用，請重新寄送，或返回登入。";
