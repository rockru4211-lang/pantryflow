...
export function countExportRows(rows: CountResult[], management: boolean) {
  return rows.map(row => ({ '品名': row.name, '區域': row.zone, '單位': row.unit, '本次數量': row.quantity, '廠商': row.supplier || '未提供', '規格': row.specification || '未提供', '原始檔': row.file_name || '未對應', '工作表': row.sheet_name || '未對應', '來源列': row.source_row || '', '盤點人': row.entered_by || '未提供', '送出時間': row.entered_at, ...(management ? {'期初': row.opening_quantity ?? '未提供'} : {}) }));
}
