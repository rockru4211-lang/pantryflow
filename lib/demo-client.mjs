...
const management = () => identity.admin || business().spaces[selectedStore]?.members.find(m=>m.user_id===userId())?.can_manage_business === true;
const canMaintain = () => management() || identity.role === 'SUPERVISOR' || (identity.role === 'LOGISTICS' && identity.businessType === 'SINGLE_RESTAURANT');
...
function workspace(id, section) {
  ...
  if (section === 'catalog' || section === 'suppliers') return {editable:canMaintain(),products:s.products,suppliers:s.suppliers};
  if (section === 'mappings') return {editable:canMaintain(),lines:[],products:s.products.map(p=>({...p,unit:p.base_unit})),rules:[]};
  if (['business','settings'].includes(section)) return {organization:b.organization, stores:b.stores, settings:b.stores.find(x=>x.id===id).settings, editable:management()||identity.role==='SUPERVISOR', revision:b.stores.find(x=>x.id===id).settings_revision, devices:[]};
  if (['members','permissions'].includes(section)) { requireRole(management()||identity.role==='SUPERVISOR'); return {members:s.members,zones:s.zones,candidates:[],invitations:s.invitations,delegations:s.delegations}; }
  if (section === 'audit') { requireRole(management()); return {events:s.audit}; }
  if (section === 'reports') {
    requireRole(identity.role !== 'STAFF');
...
function operation(id, action, data) {
  ...
  if (action==='member.save'||action==='member.offboard') {
    const member=s.members.find(m=>m.user_id===data.user_id);if(!member)denied();requireRole(member.user_id!==userId()&&!member.is_owner&&(management()||(identity.role==='SUPERVISOR'&&member.role==='STAFF'&&!member.can_manage_business)));
    if(action==='member.offboard')member.is_active=false;else {if(!management()&&(data.role!=='STAFF'||data.can_manage_business||data.extra_permissions?.length))denied('FORBIDDEN');Object.assign(member,data);}
    member.updated_at=stamp();audit();return member;
  }
  if(action==='delegation.create'||action==='delegation.revoke') {requireRole(management());if(action.endsWith('create'))s.delegations.push({...data,id:uid(),display_name:s.members.find(m=>m.user_id===data.user_id)?.display_name,revoked_at:null});else{const d=s.delegations.find(d=>d.id===data.id);if(d)d.revoked_at=stamp();}audit();return {saved:true};}
...
function rpc(name,args) {
  ...
  if(name==='get_pilot_count_completion')return {completed_by:actor(),paper_completed_by:actor()};
  ...
  if(name==='complete_pilot_count_zone') {
    ...
    if(s.zones.every(z=>s.progress.some(p=>p.session_id===session.id&&p.zone_id===z.id))){session.status='CLOSED';session.completed_at=stamp();}return {saved:true};
  }
  if(name==='complete_pilot_count_paper'){requireRole(fieldRole()||identity.role==='OWNER');const {session}=findSession(args.p_session_id);if(args.p_review)session.paper_reviewed_at=stamp();else session.paper_completed_at=stamp();return {saved:true};}
  ...
  if(name==='authorize_app_feature'){requireRole(identity.role!=='STAFF');return true;}
  denied();
}
