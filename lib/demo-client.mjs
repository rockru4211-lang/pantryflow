...
function space(id = selectedStore) { const value = business().spaces[id]; if (!value || !value.members.some(m=>m.user_id===userId()&&m.is_active)) denied('FORBIDDEN'); return value; }
...
function workspace(id, section, filter={}) {
  ...
  if (section === 'transfers') return {units:s.units||[],records:s.movements, stores:b.stores.filter(x=>x.id!==id), products:s.products.filter(p=>p.is_active).map(p=>({...p,unit:p.base_unit})), has_erp:b.organization.has_erp};
  if (section === 'transfer-search') return {stores:b.stores.filter(x=>x.id!==id).map(store=>({...store,products:b.spaces[store.id].products.filter(p=>p.is_active&&p.name.includes(filter.search||'')).map(p=>({name:p.name,unit:p.base_unit,stock:snapshotStock(b.spaces[store.id],p),states:(b.spaces[store.id].positions||[]).filter(x=>x.product_id===p.id&&x.quantity>0)}))}))};
...
function operation(id, action, data) {
  ...
  if(action.startsWith('stock.')){const r=stockOperation(s,action,data,{maintain:canMaintain(id),field:fieldRole()});audit();return r;}
  ...
  if (action.startsWith('movement.')) {
    ...
    if(action==='movement.create') {
      ...
      const p=s.products.find(p=>p.id===data.product_id&&p.is_active);if(data.product_id&&!p)denied('INVALID_PRODUCT');const unit=normalizedUnit(data.unit);const qty=quantity(data.quantity);postStock(s,p,unit,data.mode==='loan'?qty:-qty);s.units=[...new Set([...(s.units||[]),unit])];
...
function saveExpiry(id, action, data) {
  ...
  if(action==='REMINDER')s.items.unshift({...result,category:data.expires_on<=day()?'urgent':'upcoming',source:'REMINDER',unit:data.unit||item.unit||null});
...
function table(name) {
  const filters=[];let single=false,limit=Infinity,offset=0,order;
  const read=()=>{
    const spaces=Object.values(business().spaces);
    const tables={store_memberships:()=>spaces.flatMap(s=>s.members.map(m=>({...m,store_id:s.zones[0].store_id}))),staff_identities:()=>spaces.flatMap(s=>s.members),count_zones:()=>spaces.flatMap(s=>s.zones.map(z=>({...z,zone_products:z.zone_products.map(zp=>({...zp,products:s.products.find(p=>p.id===zp.product_id)}))}))),inventory_count_sessions:()=>spaces.flatMap(s=>s.sessions),count_entries:()=>spaces.flatMap(s=>s.results),count_zone_progress:()=>spaces.flatMap(s=>s.progress),count_drafts:()=>spaces.flatMap(s=>s.drafts),inventory_count_discrepancies:()=>spaces.flatMap(s=>s.discrepancies),products:()=>spaces.flatMap(s=>s.products.map(p=>({...p,organization_id:business().organization.id}))),inventory_import_files:()=>spaces.flatMap(s=>s.importFiles||[]),inventory_import_rows:()=>spaces.flatMap(s=>s.importRows||[]),inventory_import_batches:()=>[]};
    if(!tables[name])denied();let rows=tables[name]().filter(r=>filters.every(f=>f(r)));
    if(order)rows.sort((a,b)=>String(a[order.key]).localeCompare(String(b[order.key]))*(order.ascending?1:-1));rows=rows.slice(offset,offset+limit);return single?rows[0]||null:rows;
  };
  const query={...result(read),select(){return this;},eq(key,value){filters.push(r=>r[key]===value);return this;},in(key,values){filters.push(r=>values.includes(r[key]));return this;},is(key,value){filters.push(r=>r[key]===value);return this;},gte(key,value){filters.push(r=>r[key]>=value);return this;},lte(key,value){filters.push(r=>r[key]<=value);return this;},order(key,options={}){order={key,ascending:options.ascending!==false};return this;},range(a,b){offset=a;limit=b-a+1;return this;},limit(n){limit=n;return this;},maybeSingle(){single=true;return this;},single(){single=true;return this;}};
  return query;
}
...
async function demoFunction(name,body){if(!body||name!=='manage-staff')denied('DEMO_UNAVAILABLE');const id=body.storeId||selectedStore;const s=space(id);requireRole(management(id)||identity.role==='SUPERVISOR');if(!['create','reset_pin'].includes(body.action))denied('DEMO_UNAVAILABLE');s.staffInvites ||= [];let member;
 if(body.action==='reset_pin'){member=s.members.find(m=>m.user_id===body.staffId&&m.uses_pin&&m.is_active);if(!member||member.user_id===userId()||(!management(id)&&member.role!=='STAFF'))denied('FORBIDDEN');}
 else{if(body.role!=='STAFF'||!body.displayName?.trim()||!body.loginIdentifier?.trim())denied('INVALID_STAFF_INPUT');if(s.members.some(m=>m.login_identifier===body.loginIdentifier))denied('STAFF_ALREADY_EXISTS');member={user_id:uid(),display_name:body.displayName,login_identifier:body.loginIdentifier,role:'STAFF',is_active:true,is_owner:false,can_manage_business:false,uses_pin:true,email:null,extra_permissions:[],zone_ids:[],updated_at:stamp(),pending_count:0};s.members.push(member);}
...
export function createDemoClient() {
  return {rpc(name,args={}){return result(()=>{restore();const key=args.p_request_id?`${identity.businessType}:${name}:${args.p_request_id}`:null;const signature=JSON.stringify({actor:userId(),args});if(key&&requests.has(key)){const saved=requests.get(key);if(saved.signature!==signature)denied('REQUEST_CONFLICT');return saved.data;}const before=clone(businesses);try{const data=rpc(name,args);if(key)requests.set(key,{signature,data:clone(data)});persist();return data;}catch(error){businesses=before;throw error;}});},from:table,
    auth:new Proxy({}, {get:()=>()=>result(()=>denied())}),
    functions:{invoke:(name,options)=>result(()=>demoFunction(name,options.body))},
    storage:{from:bucket=>({upload:(path,bytes)=>result(()=>{if(bucket!=='inventory-imports')denied();requireRole(canMaintain());if(!path.includes(`/${selectedStore}/`))denied('FORBIDDEN');uploads.set(path,new Blob([bytes]));return {path};}),download:path=>result(()=>{if(!uploads.has(path))denied('示範原檔已不在此分頁，請重新選擇同一檔案。');return uploads.get(path);}),createSignedUrl:()=>result(()=>denied())})}};
}
