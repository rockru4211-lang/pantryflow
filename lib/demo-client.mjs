...
export function createDemoClient() {
  return {rpc(name,args={}){return result(()=>{restore();space(args.p_store_id||selectedStore);if(args.p_session_id)findSession(args.p_session_id);const key=args.p_request_id?`${identity.businessType}:${name}:${args.p_request_id}`:null;const signature=JSON.stringify({actor:userId(),args});if(key&&requests.has(key)){const saved=requests.get(key);if(saved.signature!==signature)denied('REQUEST_CONFLICT');return saved.data;}const before=clone(businesses);try{const data=rpc(name,args);if(key)requests.set(key,{signature,data:clone(data)});persist();return data;}catch(error){businesses=before;throw error;}});},from:table,
