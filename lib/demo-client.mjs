import {pendingReceiptErp} from './receipt-delivery.ts';
...
function receiptLedger(id) {
  const s=space(id);
  const asText=value=>value===null||value===undefined?null:typeof value==='object'?JSON.stringify(value):String(value);
  const numeric=value=>value!==null&&/^-?[0-9]+(\.[0-9]+)?$/.test(value)?Number(value):null;
  const rows=s.batches.flatMap(batch=>{
    if(!batch.ocr_status)return [];
    const fields=s.fields[batch.id]||[];
    const value=(key,name)=>asText(fields.find(field=>field.row_key===key&&field.field_name===name)?.value);
    const mappings=batch.mappings||s.products.slice(0,2).map((p,i)=>({row_key:`row-${i}`,product_id:p.id}));
    return [...new Set(fields.filter(field=>field.row_key!=='document').map(field=>field.row_key))].map(key=>{
      const mapping=mappings.find(item=>item.row_key===key),product=s.products.find(item=>item.id===mapping?.product_id);
      const q=numeric(value(key,'quantity')),price=numeric(value(key,'unit_price_ex_tax'));
      return {batch_id:batch.id,run_id:`${batch.id}-ocr`,row_key:key,uploaded_at:batch.uploaded_at,
        receipt_date:value('document','receipt_date')??batch.work_date??null,supplier_name:value('document','supplier_name')||'未提供',
        product_code:product?.product_code??null,product_id:product?.id??null,product_name:product?.name??value(key,'product')??'未命名品項',source_product:value(key,'product')??'未提供',
        specification:(value(key,'specification')||null)??product?.specification??'未提供',unit:(value(key,'unit')||null)??product?.count_unit??product?.base_unit??'未提供',
        quantity:q,unit_price:price,subtotal:q===null||price===null?null:q*price,mapped:!!mapping?.product_id,
        status:batch.review_saved||batch.status==='COMPLETED'?'COMPLETE':mapping?.product_id?'PENDING':'NEEDS_MAPPING',review_allowed:reviewRole(id)};
    });
  });
  const descending=(a,b)=>a===b?0:a===null?1:b===null?-1:a>b?-1:1;
  return rows.sort((a,b)=>descending(a.receipt_date,b.receipt_date)||descending(a.uploaded_at,b.uploaded_at)||a.row_key.localeCompare(b.row_key));
}
...
function operation(id, action, data) {
  ...
  if(action==='receipt.delivery'||action==='receipt.erp-bulk'){
    requireRole(['STAFF','SUPERVISOR'].includes(memberFor(id)?.role));
    if(action==='receipt.erp-bulk'){
      if(!Array.isArray(data.batch_ids)||data.batch_ids.length<1||data.batch_ids.length>100)denied('INVALID_RECEIPTS');
      const selected=[...new Set(data.batch_ids)].map(key=>s.batches.find(b=>b.id===key));
      if(selected.some(b=>!b||!b.erp_required))denied('RECEIPT_ACCESS_DENIED');
      if(selected.some(b=>!b.job_status))denied('ORIGINAL_UPLOAD_INCOMPLETE');
      for(const batch of selected)if(!batch.erp_completed_at){batch.erp_completed_at=stamp();batch.erp_completed_by=actor();}
      audit();return {saved:true,count:selected.length};
    }
    const batch=s.batches.find(b=>b.id===data.batch_id);if(!batch)denied('RECEIPT_ACCESS_DENIED');
    const old=batch.delivery||{revision:0,issues:[]};if(data.revision!==old.revision)denied('REVISION_CONFLICT');
    if(data.arrived_time&&!data.arrived_on)denied('INVALID_ARRIVAL');
    if(data.arrived_on&&(!/^\d{4}-\d{2}-\d{2}$/.test(data.arrived_on)||new Date(`${data.arrived_on}T00:00:00Z`).toISOString().slice(0,10)!==data.arrived_on))denied('INVALID_ARRIVAL');
    if(data.arrived_time&&!/^([01]\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(data.arrived_time))denied('INVALID_ARRIVAL');
    if(!Array.isArray(data.issues)||data.issues.length>100||new Set(data.issues.map(i=>i.id)).size!==data.issues.length)denied('INVALID_ISSUES');
    for(const issue of data.issues){
      if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(issue.id)||!issue.name?.trim()||issue.name.length>160||!['未收到','少收貨','多收貨','效期太短','其他'].includes(issue.reason)||!['OPEN','COMPLETE'].includes(issue.status)||(issue.unit?.length||0)>30||(issue.note?.length||0)>1000||((issue.status==='COMPLETE'||issue.reason==='其他')&&!issue.note?.trim()))denied('INVALID_ISSUES');
      if(issue.quantity!=null&&(typeof issue.quantity!=='number'||!Number.isFinite(issue.quantity)||issue.quantity<0||issue.quantity>=1000000000||!issue.unit?.trim()))denied('INVALID_QUANTITY');
    }
    if(old.issues.some(i=>!data.issues.some(n=>n.id===i.id)))denied('INVALID_ISSUE_REMOVAL');
    batch.delivery={arrived_on:data.arrived_on||null,arrived_time:data.arrived_time||null,issues:clone(data.issues),revision:old.revision+1};audit();return clone(batch.delivery);
  }
...
function rpc(name,args) {
  ...
  if(name==='get_pilot_receipt_ledger'||name==='confirm_pilot_receipt_ledger'){
    if(!['SUPERVISOR','LOGISTICS','OWNER'].includes(memberFor(id)?.role))denied('RECEIPT_REVIEWER_REQUIRED');
    if(name==='get_pilot_receipt_ledger')return receiptLedger(id);
    if(!Array.isArray(args.p_rows))denied('ROWS_REQUIRED');
    let confirmed=0;const failed=[];
    for(const item of args.p_rows){
      const before=clone(businesses);
      try{
        const s=space(id),batch=s.batches.find(b=>b.id===item?.batch_id);
        if(!batch)denied('STORE_SCOPE_MISMATCH');
        if(!reviewRole(id))denied('RECEIPT_REVIEWER_REQUIRED');
        if(!batch.ocr_status||item.run_id!==`${batch.id}-ocr`)denied('OCR_VERSION_CHANGED');
        if(batch.ocr_status!=='SUCCEEDED')denied('OCR_NOT_READY');
        if(item.row_key==='document'||!(s.fields[batch.id]||[]).some(field=>field.row_key===item.row_key))denied('OCR_LINE_NOT_FOUND');
        // Published receipts are already saved. Replaying them must not post stock again.
        if(!batch.review_saved&&batch.status!=='COMPLETED')rpc('save_pilot_receipt_review',{p_store_id:id,p_batch_id:batch.id,p_run_id:item.run_id,p_row_key:item.row_key});
        confirmed++;
      }catch(error){
        businesses=before;
        failed.push({batch_id:item?.batch_id??null,row_key:item?.row_key??null,error:error.message});
      }
    }
    return {confirmed,failed_count:failed.length,failed};
  }
