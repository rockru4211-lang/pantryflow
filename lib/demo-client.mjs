...
function space(id = selectedStore) { const value = business().spaces[id]; if (!business().stores.some(x=>x.id===id&&x.is_active) || !value || !value.members.some(m=>m.user_id===userId()&&m.is_active)) denied('FORBIDDEN'); return value; }
...
function workspace(id, section, filter={}) {
  ...
  if (['business','settings'].includes(section)) return {...(section==='business'?{organization:b.organization,stores:b.stores.filter(x=>memberFor(x.id)?.is_active&&management(x.id))}:{}), settings:b.stores.find(x=>x.id===id).settings, editable:management(id), revision:b.stores.find(x=>x.id===id).settings_revision, devices:[]};
  if (['members','permissions'].includes(section)) { requireRole(policy(id).can_manage_members); return {members:s.members,zones:s.zones,candidates:management(id)?[...new Map(Object.values(b.spaces).flatMap(x=>x.members).filter(m=>m.is_active&&!s.members.some(h=>h.user_id===m.user_id)).map(m=>[m.user_id,m])).values()]:[],invitations:s.invitations,delegations:s.delegations}; }
...
function operation(id, action, data) {
  ...
  if(action==='store.create'){
    ...
    const owners=[...new Map(Object.values(b.spaces).flatMap(s=>s.members).filter(m=>m.is_active&&m.can_manage_business&&m.role==='OWNER').map(m=>[m.user_id,m])).values()];
...
function rpc(name,args) {
  const id=args.p_store_id||selectedStore;space(id);
  ...
  if(name==='correct_pilot_receipt_field'){requireRole(fieldRole());for(const s of authorizedSpaces())for(const fields of Object.values(s.fields)){const f=fields.find(f=>f.id===args.p_field_id);if(f){f.value=args.p_value;f.corrected=true;return {saved:true};}}denied();}
...
async function demoFunction(name,body){if(!body||name!=='manage-staff')denied('DEMO_UNAVAILABLE');const id=body.storeId||selectedStore;const s=space(id);requireRole(policy(id).can_manage_members);if(!['create','reset_pin'].includes(body.action))denied('DEMO_UNAVAILABLE');s.staffInvites ||= [];let member;
...
export function createDemoClient() {
  return {rpc(name,args={}){return result(()=>{restore();space(args.p_store_id||selectedStore);if(args.p_session_id)findSession(args.p_session_id);const key=args.p_request_id?`${identity.businessType}:${name}:${args.p_request_id}`:null;const signature=JSON.stringify({actor:userId(),args});if(name==='app_operation'&&/^(store\.|settings\.|business\.|delegation\.|invite\.)/.test(args.p_action))requireRole(management(args.p_store_id));if(name==='app_operation'&&args.p_action.startsWith('member.'))requireRole(policy(args.p_store_id).can_manage_members);if(key&&requests.has(key)){const saved=requests.get(key);if(saved.signature!==signature)denied('REQUEST_CONFLICT');return saved.data;}const before=clone(businesses);try{const data=rpc(name,args);if(key)requests.set(key,{signature,data:clone(data)});persist();return data;}catch(error){businesses=before;throw error;}});},from:table,
    auth:new Proxy({}, {get:()=>()=>result(()=>denied())}),
    functions:{invoke:(name,options)=>result(()=>demoFunction(name,options.body))},
    storage:{from:bucket=>({upload:(path,bytes)=>result(async()=>{authorizeDemoFile(bucket,path);await saveDemoFile(path,bytes);return {path};}),download:path=>result(()=>{authorizeDemoFile(bucket,path);return loadDemoFile(path);}),createSignedUrl:(path,seconds)=>result(async()=>{authorizeDemoFile(bucket,path);return {signedUrl:await demoFileUrl(path,seconds)};})})}};
}
function authorizeDemoFile(bucket,path){if(bucket!=='inventory-imports')denied();const [org,storeId]=path.split('/');if(org!==business().organization.id)denied('FORBIDDEN');space(storeId);requireRole(canMaintain(storeId));}
