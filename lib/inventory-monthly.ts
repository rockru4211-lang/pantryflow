...
export type InventoryMonth = {
 ...
 sessions:{id:string;completed_at:string;status:string}[];source_id:string|null;completed_at:string|null;source_complete:boolean;has_active_count:boolean;
...
export function inventoryCategories(rows:InventoryRow[]) {
 ...
 return [...groups.values()].map(g=>({...g,amount:Math.round(g.amount*100)/100})).sort((a,b)=>b.amount-a.amount||a.name.localeCompare(b.name,'zh-TW'));
}
export function inventoryExportRows(rows:InventoryRow[]) {return rows.map(r=>({
 ...
 '上月數量':r.previous_quantity??'未提供','本月數量':r.current_quantity??'未盤','數量增減':r.difference??comparisonLabel(r),
