...
// Keep the parser and worker on the same polyfilled build for Safari/iPad.
import workerUrl from 'pdfjs-dist/legacy/build/pdf.worker.min.mjs?url';
export async function readInventoryPdf(data:ArrayBuffer) {
 const pdfjs=await import('pdfjs-dist/legacy/build/pdf.mjs');pdfjs.GlobalWorkerOptions.workerSrc=workerUrl;
