export type OwnerSetupDraft = {
  ...
  store_name: string; store_code: string; staff_login_mode: string;
};
...
export function parseOwnerSetup(value: unknown): OwnerSetup {
  ...
  return { required: v.required, step: v.step!, revision: v.revision || 0, draft: {
    ...
    store_name: d?.store_name || "", store_code: d?.store_code || "", staff_login_mode: d?.staff_login_mode || "NAME_OR_NICKNAME",
  } };
}
