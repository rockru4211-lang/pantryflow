...
function renderLogin(){root.innerHTML=loginPage();bindError(document.querySelector('#management-login'),async form=>{await signIn(form.get('email'),form.get('password'));await boot()})}
...
async function renderPage(page,context={}){state.page=page;state.context=context;const role=state.profile.is_owner?'OWNER':state.store.role;const canManage=state.profile.is_owner||['ADMIN','SUPERVISOR'].includes(state.store.role);let content;
...
function renderUnassignedStore(){root.innerHTML=unassignedStorePage();document.querySelector('[data-sign-out]').onclick=async()=>{await signOut();renderLogin()}}
