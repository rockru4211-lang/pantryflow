...
function renderLogin(view='identity',context={}){
  ...
  bindError(document.querySelector('#owner-registration'),async form=>{if(form.get('password')!==form.get('passwordConfirm'))throw new Error('PASSWORD_CONFIRM_MISMATCH');const result=await signUpOwner(form.get('displayName'),form.get('email'),form.get('password'));if(result.session)return boot();renderLogin('register',{message:'驗證信已寄出。請到信箱完成 Email 驗證，再回到 PantryFlow 建立商家與門市。'})},{log:false});
...
listenAuthChanges(event=>{if(event==='PASSWORD_RECOVERY')renderLogin('update-password')});
