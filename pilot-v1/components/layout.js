...
export function layout({storeName,role,page,content,displayName}){
  ...
  return `<div class="shell ${roleClasses[role]||'role-staff'}"><header class="app-topbar"><button class="store-switch" data-store-switch type="button" aria-label="切換門市"><span>${escapeHtml(storeName||'封閉 Pilot')}</span><b>⌄</b></button><div class="topbar-brand"><span class="mini-mark">P</span><strong>PantryFlow</strong></div><button class="notification-button" data-route="notifications" type="button" aria-label="通知">◔</button></header><div class="home-greeting"><p>${escapeHtml(displayName||'')}，歡迎回來</p><span class="role-pill">${escapeHtml(roleLabels[role]||role||'')}</span></div>${content}</div><nav class="bottom-nav" aria-label="主要導覽">${items.map(([id,label])=>`<button data-route="${id}" class="${page===id?'active':''}" aria-current="${page===id?'page':'false'}">${label}</button>`).join('')}</nav>`;
}
