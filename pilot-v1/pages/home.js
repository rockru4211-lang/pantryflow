...
const workItems=[['盤點','count','▣'],['進貨','receiving','▤'],['廢棄','waste','♲'],['效期巡檢','expiry','◷'],['其他作業','other','•••']];
const reviewItems=[['收貨待核對',2,'張','purple'],['編碼待確認',1,'筆','blue'],['貨單差異',2,'張','orange'],['盤點異常',3,'筆','green']];
const managementItems=[['商品／編碼','catalog','▣'],['供應商','suppliers','♙'],['配方','recipes','♨'],['成本分析','costs','◉'],['報表中心','reports','▥'],['設定','profile','⚙']];
const ownerItems=[['成員與權限','members','♙'],['商家設定','business','▣'],['模組開關','modules','✣'],['配方機密權限','recipe-access','♙'],['資料匯出','export','⇩'],['Audit Log','audit','▤']];
...
function metricCards(items){return `<div class="metric-grid">${items.map(([label,itemValue,tone,icon])=>`<button class="metric-card tone-${tone}" type="button"><i>${icon}</i><span>${escapeHtml(label)}</span><strong>${count(itemValue)}</strong><small>項</small></button>`).join('')}</div>`}
...
function adminGrid(items){return `<div class="admin-grid">${items.map(([label,route,icon])=>`<button class="admin-card" data-feature="${route}" type="button"><span>${icon}</span><strong>${escapeHtml(label)}</strong></button>`).join('')}</div>`}
...
function employeeHome(data){const alerts=data.alerts||{};return `<main class="home-page employee-home" data-home-kind="employee">
  <section class="home-section"><h1>今天先看</h1>${metricCards([['缺貨風險',value(alerts.stockRisk,3),'danger','!'],['即期提醒',value(alerts.expiry,2),'warning','◷'],['待確認',value(alerts.pending,1),'info','?']])}</section>
