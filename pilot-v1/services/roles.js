...
export function countAccessForRole(value){const role=normalizeStoreRole(value);if(role==='STAFF')return{kind:'staff',canCount:true,canConfigure:false,canReview:false,canAnalyze:false,canGovern:false};if(role==='ADMIN'||role==='SUPERVISOR')return{kind:'manager',canCount:true,canConfigure:true,canReview:true,canAnalyze:false,canGovern:false};if(role==='LOGISTICS')return{kind:'logistics',canCount:false,canConfigure:false,canReview:false,canAnalyze:true,canGovern:false};if(role==='OWNER')return{kind:'owner',canCount:false,canConfigure:false,canReview:false,canAnalyze:false,canGovern:true};return{kind:null,canCount:false,canConfigure:false,canReview:false,canAnalyze:false,canGovern:false}}
