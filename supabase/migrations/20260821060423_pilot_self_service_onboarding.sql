-- PantryFlow Pilot v0.1: public sign-up, self-service organization onboarding,
-- and organization-scoped catalog administration.

alter table public.profiles
  alter column organization_id drop not null,
  alter column role drop not null,
  alter column store set default '';
...
create trigger on_auth_user_created_pantryflow
...
begin
  ...
  insert into public.profiles (id, display_name, organization_id, role, store)
  values (v_user_id, '', v_org_id, 'ADMIN', v_name)
  on conflict (id) do update set
...
begin
  for p in
    ...
    where schemaname = 'public'
      and tablename in ('organizations', 'profiles', 'organization_members', 'products', 'count_zones', 'zone_products')
  loop
...
for select to authenticated
using (id = (select auth.uid()) or organization_id = public.current_organization_id());

create policy profiles_self_update on public.profiles
for update to authenticated
using (id = (select auth.uid()))
with check (id = (select auth.uid()) and organization_id = public.current_organization_id());
...
create policy count_zones_org_select on public.count_zones
for select to authenticated
using (organization_id = public.current_organization_id());
create policy count_zones_admin_insert on public.count_zones
for insert to authenticated
with check (organization_id = public.current_organization_id() and public.is_admin());
create policy count_zones_admin_update on public.count_zones
for update to authenticated
using (organization_id = public.current_organization_id() and public.is_admin())
with check (organization_id = public.current_organization_id() and public.is_admin());

create policy zone_products_org_select on public.zone_products
for select to authenticated
using (exists (
  select 1 from public.count_zones z
  where z.id = zone_id and z.organization_id = public.current_organization_id()
));
create policy zone_products_admin_insert on public.zone_products
for insert to authenticated
with check (
  public.is_admin()
...
create policy zone_products_admin_update on public.zone_products
for update to authenticated
using (
  public.is_admin()
  and exists (select 1 from public.count_zones z where z.id = zone_id and z.organization_id = public.current_organization_id())
...
grant select on public.organizations, public.profiles, public.organization_members,
  public.products, public.suppliers, public.count_zones, public.zone_products,
  public.inventory_count_sessions, public.count_zone_progress, public.count_entries,
  public.count_drafts, public.inventory_count_discrepancies to authenticated;
grant insert, update on public.products, public.count_zones, public.zone_products to authenticated;
grant insert, update on public.inventory_count_sessions, public.count_zone_progress,
  public.count_entries, public.count_drafts, public.inventory_count_discrepancies to authenticated;
