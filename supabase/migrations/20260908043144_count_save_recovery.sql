...
declare v_entry jsonb; v_stamp timestamptz; v_results jsonb:='[]'::jsonb;
begin
 if jsonb_typeof(p_entries) is distinct from 'array' or jsonb_array_length(p_entries)>1000 then raise exception 'INVALID_DRAFT_BATCH'; end if;
 for v_entry in select value from jsonb_array_elements(p_entries) loop
   v_stamp:=public.save_pilot_count_draft(p_session_id,(v_entry->>'zone_id')::uuid,(v_entry->>'product_id')::uuid,(v_entry->>'quantity')::numeric,(v_entry->>'expected_updated_at')::timestamptz);
   v_results:=v_results||jsonb_build_array(jsonb_build_object('zone_id',v_entry->>'zone_id','product_id',v_entry->>'product_id','updated_at',v_stamp));
 end loop;
 return v_results;
end; $$;
revoke all on function public.save_pilot_count_drafts(uuid,jsonb) from public,anon;
grant execute on function public.save_pilot_count_drafts(uuid,jsonb) to authenticated;
