-- Store-scoped administration; preserve work history, credentials and explicit revocations.
alter table public.store_memberships add column can_manage_business boolean;
alter table public.store_memberships add column display_name text;
update public.store_memberships sm set can_manage_business=om.can_manage_business or coalesce(sm.work_role,sm.role)='OWNER' or (coalesce(sm.work_role,sm.role) in ('SUPERVISOR','ADMIN') and o.business_type::text='SINGLE_RESTAURANT'), display_name=si.display_name
from public.organization_members om,public.organizations o,public.staff_identities si
where om.organization_id=sm.organization_id and om.user_id=sm.user_id and o.id=sm.organization_id and si.organization_id=sm.organization_id and si.user_id=sm.user_id;
update public.store_memberships set can_manage_business=false where can_manage_business is null;
alter table public.store_memberships alter column can_manage_business set not null;
...
begin
 if tg_table_name='organization_members' then
  ...
  new.can_manage_business:=coalesce(new.can_manage_business,(select om.can_manage_business from public.organization_members om where om.organization_id=new.organization_id and om.user_id=new.user_id),false);
...
create or replace function private.can_manage_business(p_store uuid,p_actor uuid default auth.uid()) returns boolean language sql stable security definer set search_path='' as $$
 ...
 and (sm.can_manage_business or coalesce(sm.work_role,sm.role)='OWNER')),false)
$$;
create or replace function private.can_manage_members(p_store uuid,p_actor uuid default auth.uid()) returns boolean language sql stable security definer set search_path='' as $$
 select private.can_manage_business(p_store,p_actor) or (private.member_active(p_store,p_actor) and (p_actor is distinct from auth.uid() or private.app_session_valid(p_store)) and exists(select 1 from public.store_memberships sm where sm.store_id=p_store and sm.user_id=p_actor and coalesce(sm.work_role,sm.role) in ('SUPERVISOR','ADMIN')))
$$;
create or replace function private.assignable_member_roles(p_store uuid,p_actor uuid default auth.uid()) returns text[] language sql stable security definer set search_path='' as $$
...
$$;
revoke all on function private.can_manage_members(uuid,uuid),private.assignable_member_roles(uuid,uuid) from public,anon,authenticated;
-- Repair only missing owner associations. Existing disabled/limited memberships stay intact.
...
select s.id,s.organization_id,om.user_id,'owner-'||om.user_id::text,'OWNER','OWNER',s.created_by,true
from public.stores s join public.organization_members om on om.organization_id=s.organization_id
where s.is_active and om.is_active and coalesce(om.work_role,om.role)='OWNER'
...
AS $function$ declare result jsonb;begin
 ...
 'role',private.app_role(s.id),'can_manage_business',private.can_manage_business(s.id),'can_manage_stores',private.can_manage_business(s.id),'can_manage_members',private.can_manage_members(s.id),'assignable_roles',private.assignable_member_roles(s.id),'is_business_responsible',o.owner_user_id=auth.uid(),'permissions',jsonb_build_object('reports_view',private.has_app_feature(s.id,'REPORTS_VIEW'),'data_export',private.has_app_feature(s.id,'DATA_EXPORT')),'settings',coalesce(st.settings,'{}'::jsonb),
...
begin
 ...
 if p_action='member.offboard' then
  ...
  values(p_store,v_org,v_target,btrim(p_data->>'login_identifier'),(p_data->>'role')::public.app_role,coalesce((p_data->>'is_active')::boolean,true),auth.uid(),(p_data->>'role')::public.app_role,false)
...
begin
 if p_action='store.create' then
  ...
  where om.organization_id=v_org and om.is_active and coalesce(om.work_role,om.role)='OWNER' on conflict(store_id,user_id) do nothing;
...
-- Owners cover missing stores, while explicit per-store roles and revocations remain unchanged.
...
begin
 if new.is_active and coalesce(new.work_role,new.role)='OWNER' then
  insert into public.store_memberships(store_id,organization_id,user_id,login_identifier,role,work_role,assigned_by,can_manage_business,display_name)
...
revoke all on function private.complete_owner_store_scope() from public,anon,authenticated;
create trigger complete_owner_store_scope after insert or update of work_role,is_active on public.store_memberships for each row execute function private.complete_owner_store_scope();
-- A PIN belongs to the account. Resetting it must not grant control over a role
-- the requester cannot manage in another store.
create or replace function public.reset_staff_activation(p_user_id uuid,p_code text,p_actor uuid,p_store_id uuid) returns void language plpgsql security definer set search_path='' as $$
begin
 if p_actor=p_user_id or length(p_code)<32 or not private.can_manage_members(p_store_id,p_actor) or not private.member_active(p_store_id,p_user_id) then raise exception 'ACTIVATION_NOT_ALLOWED' using errcode='42501';end if;
 if exists(select 1 from public.organization_members om where om.user_id=p_user_id and om.is_owner)
 or exists(select 1 from public.store_memberships sm where sm.user_id=p_user_id and sm.is_active and (
 not coalesce(coalesce(sm.work_role,sm.role)::text=any(private.assignable_member_roles(sm.store_id,p_actor)),false)
 or (sm.can_manage_business and not private.can_manage_business(sm.store_id,p_actor)))) then raise exception 'ACTIVATION_NOT_ALLOWED' using errcode='42501';end if;
