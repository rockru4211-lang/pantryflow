alter table private.store_movements
  ...
  add column if not exists unit_price_snapshot numeric check (unit_price_snapshot is null or unit_price_snapshot >= 0),
  add column if not exists amount_snapshot numeric check (amount_snapshot is null or amount_snapshot >= 0),
...
begin
  ...
  if v_other is null or v_other=p_store
     or not exists(select 1 from public.stores where id=v_other and organization_id=v_org and is_active) then
...
begin
  ...
  src:=replace(
    ...
    'elsif p_action=''movement.transfer-create'' then v_result:=private.create_store_transfer(p_store,p_data);v_id:=nullif(v_result->>''id'','''')::uuid;'||chr(10)||' elsif p_action in (''movement.create'',''movement.return'',''movement.exchange'') then'
  );
  if src=original then raise exception 'TRANSFER_OPERATION_PATCH_MISSING'; end if;
  execute src;
  ...
  src:=replace(
    ...
    'if p_section=''transfers'' then return private.transfers_workspace_v2(p_store,p_filter);end if;'||chr(10)||' if p_section=''mappings'' then return private.app_mapping_workspace(p_store);end if;'
  );
  if src=original then raise exception 'TRANSFER_WORKSPACE_PATCH_MISSING'; end if;
  execute src;
end $$;
