...
test('database migrations and generated types contain the beta schema contract', async () => {
  ...
  const contract = await readFile(new URL('../supabase/migrations/20260905153515_supplier_inventory_import_results.sql', import.meta.url), 'utf8');
  const config = await readFile(new URL('../supabase/config.toml', import.meta.url), 'utf8');
  assert.match(types, /get_app_schema_version/);
  assert.match(contract, /20260905_merchant_beta_v5/);
