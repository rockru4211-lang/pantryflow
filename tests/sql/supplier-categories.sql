...
declare owner_id uuid; person uuid; org uuid; sid uuid; foreign_store uuid; supplier_id uuid; req uuid;
 data jsonb; payload jsonb; result jsonb; saved jsonb; before_data text; before_products text; mode text; r text; denied boolean;
begin
 select md5(coalesce(string_agg(to_jsonb(d)::text,'' order by d.id),'')) into before_data from public.receipt_documents d;
 select md5(coalesce(string_agg(to_jsonb(p)::text,'' order by p.id),'')) into before_products from public.products p;
 ...
 foreach mode in array array['SINGLE_RESTAURANT','CHAIN_RESTAURANT'] loop
  ...
  foreach r in array array['OWNER','LOGISTICS','SUPERVISOR','STAFF'] loop
   ...
   payload:=jsonb_build_object('supplier_category','酒類','name','Supplier '||r,'contact_name','QA','phone','02-00000000','delivery_note','每週一、四','order_method','LINE','order_url','https://example.com/order','cutoff_time','前日15:00','order_note','缺貨先確認','aliases',jsonb_build_array('OCR '||r));
   ...
   assert saved->>'supplier_category'='酒類','supplier category roundtrip failed';
   assert saved->>'order_note'='缺貨先確認' and saved->>'cutoff_time'='前日15:00','contact roundtrip failed';
   payload:=payload||jsonb_build_object('id',supplier_id,'updated_at',saved->>'updated_at','supplier_category','食材','name','Renamed '||r,'order_note','更新備註');
   result:=public.app_operation(sid,'supplier.save',payload,gen_random_uuid());
   assert result->'previous'->>'supplier_category'='酒類' and result->'value'->>'supplier_category'='食材','category history lost';
   ...
   assert result->'value'->>'supplier_category'='食材','legacy save erased category';
   denied:=false;begin perform public.app_operation(sid,'supplier.save',jsonb_build_object('id',supplier_id,'name','Renamed '||r,'updated_at',result->'value'->>'updated_at','supplier_category','酒水'),gen_random_uuid());exception when invalid_parameter_value then denied:=true;end;assert denied,'product category accepted as supplier category';
   denied:=false;begin perform public.app_operation(sid,'supplier.save',jsonb_build_object('id',supplier_id,'name','Renamed '||r,'updated_at',result->'value'->>'updated_at','supplier_category',null),gen_random_uuid());exception when invalid_parameter_value then denied:=true;end;assert denied,'null category accepted';
   assert (select new_value->'value'->>'supplier_category' from public.audit_logs where entity_id=supplier_id::text and action='supplier.save' and new_value->'value'->>'supplier_category'='食材' limit 1)='食材','category audit missing';
   -- A legacy supplier without a details row remains visible as unclassified.
   insert into public.suppliers(organization_id,supplier_code,name) values(org,'legacy-'||r,'Legacy '||r) returning id into supplier_id;
   select x into saved from jsonb_array_elements(public.app_workspace(sid,'suppliers')->'suppliers') x where x->>'id'=supplier_id::text;
   assert saved->>'supplier_category'='待分類','legacy supplier default missing';
   if r='LOGISTICS' then
 ...
 assert before_products=(select md5(coalesce(string_agg(to_jsonb(p)::text,'' order by p.id),'')) from public.products p),'supplier classification changed products';
